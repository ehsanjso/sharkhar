...
async function main() {
  ...
  if (balance.eq(0)) {
    console.log('Nothing to redeem - already redeemed?');
    const usdc = await usdcContract.balanceOf(wallet.address);
    console.log('Current USDC.e:', ethers.utils.formatUnits(usdc, 6));
    return;
  }
  ...
  console.log('Gas price:', ethers.utils.formatUnits(feeData.maxFeePerGas || 0, 'gwei'), 'gwei');
