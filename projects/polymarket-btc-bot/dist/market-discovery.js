 ...
 */
import { timeframeToDuration } from './types-multi';
 ...
 */
export async function discoverCandleMarkets(assets = ['BTC', 'ETH', 'SOL'], timeframes = ['5min', '15min', '1hr']) {
    const markets = [];
    ...
    try {
        ...
        for (const event of events) {
            ...
            for (const asset of assets) {
                ...
                const hasAsset = assetNames.some(name => slug.includes(name.toLowerCase()) || title.includes(name.toLowerCase()));
                if (!hasAsset)
                    continue;
                ...
                for (const timeframe of timeframes) {
                    ...
                    const hasTimeframe = tfLabels.some(label => slug.includes(label.toLowerCase()) || title.includes(label.toLowerCase()));
                    if (!hasTimeframe)
                        continue;
...
function getTimeframeLabels(tf) {
    const labels = {
 ...
 */
function parseEventToMarket(event, providedAsset, providedTimeframe) {
    try {
        ...
        let upIndex = outcomes.findIndex((o) => o.toLowerCase().includes('up'));
        let downIndex = outcomes.findIndex((o) => o.toLowerCase().includes('down'));
        if (upIndex === -1)
            upIndex = 0;
        if (downIndex === -1)
            downIndex = 1;
 ...
 */
export async function watchForNewMarkets(assets, timeframes, onNewMarket, intervalMs = 30000) {
 ...
 */
export async function discoverFromPolymarket(timeframe) {
    const markets = [];
 ...
 */
export async function discoverAllMarkets(assets = ['BTC', 'ETH', 'SOL'], timeframes = ['5min', '15min', '1hr']) {
    const allMarkets = [];
...
if (isMainModule) {
    (async () => {
        ...
        const markets = await discoverAllMarkets(['BTC', 'ETH', 'SOL'], ['5min', '15min', '1hr']);
