...
└─────────────────────────────────────────┘
  `);
    ...
    while (running) {
        ...
        if (session.side) {
            ...
            if (betToPlace && !betToPlace.executed) {
                if (strategy.meetsThreshold(session, currentOdds)) {
                    ...
                    const result = await polymarket.placeMakerOrder(tokenId, price, betToPlace.amount);
                    if (result.success) {
        ...
        process.stdout.write(`\r⏱️ Min ${minutesSinceStart.toFixed(1)} | ` +
...
//# sourceMappingURL=manual-trade.js.map