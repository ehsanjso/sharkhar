...
export class NewsResearchService {
   ...
   */
  async searchBitcoinNews(customQuery?: string): Promise<ResearchReport> {
   ...
   */
  async getMarketSentiment(): Promise<ResearchReport> {
   ...
   */
  async checkBreakingNews(): Promise<ResearchReport> {
   ...
   */
  async researchEvent(event: string): Promise<ResearchReport> {
   ...
   */
  private analyzeSentiment(text: string): { direction: 'BULLISH' | 'BEARISH' | 'NEUTRAL'; confidence: number } {
...
// Singleton instance
let newsService: NewsResearchService | null = null;

export function getNewsService(apiKey?: string): NewsResearchService {
  if (!newsService) {
