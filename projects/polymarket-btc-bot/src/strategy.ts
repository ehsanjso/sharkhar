...
export class TradingStrategy {
  ...
  createSession(market: BTCMarket, btcOpenPrice: number): TradingSession {
    const bets: BetSchedule[] = this.config.betSchedule.map((b) => ({
      minute: b.minute,
      amount: b.amount,
      executed: false,
    }));
  ...
  decideSide(
    ...
    if (preferredProb >= this.config.minProbability) {
      console.log(
        `📈 Deciding ${preferredSide} - BTC ${isUp ? '+' : ''}$${priceChange.toFixed(2)}, ` +
    ...
    if (otherProb >= this.config.minProbability) {
      console.log(
        `🔄 Market disagrees - going with ${isUp ? 'DOWN' : 'UP'} at ` +
  ...
  meetsThreshold(session: TradingSession, odds: MarketOdds): boolean {
    if (!session.side) return false;

    const currentProb =
      session.side === 'UP' ? odds.upProbability : odds.downProbability;
  ...
  calculateMakerPrice(
    ...
    if (!session.side) return 0;

    // Get current market price
    const marketPrice =
      session.side === 'UP' ? odds.upPrice : odds.downPrice;
  ...
  getSummary(session: TradingSession): string {
    ...
    if (session.result !== 'PENDING') {
      ...
      lines.push(
        `Profit: ${session.profit >= 0 ? '+' : ''}$${session.profit.toFixed(2)}`
      );
    }
