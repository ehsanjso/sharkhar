...
Local commits need manual push. Remote has diverged - run:
```bash
cd ~/clawd && git pull --rebase origin main && git push
```

May need to resolve conflicts with stray files (zip files, log files).
